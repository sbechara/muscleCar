...
void setup()
{
  // In order to tell the Arduino how to treat each respective pin, we need to define something called the pin mode.
  // This pin mode can either be an input or an output. For the motors, since we are sending singals to them from the Arduino, these pins will be configured as outputs.
  // This can be easily done with the built in pinMode function.
  
  //pinMode(leftMotor, OUTPUT);
  //pinMode(rightMotor, OUTPUT);

  // We don't want the vehicle driving off on its upon the start of the program, so we need to tell the Arduino to turn both motors off.
  // This can be easily done using the digitalWrite function, giving the left and right motors a value of 0, or LOW.
  
  //digitalWrite(leftMotor, LOW);
  //digitalWrite(rightMotor, LOW);

  // Finally, we need to set up the RF receiver. We can do this by using the built in functions below to set the receiver pin receving rate.
...
void loop() 
{
  ...
  if(vw_get_message(buf, &buflen)) 
  {
    ...
    // This is done with another built-in function "analogWrite". Essentially, the left and right motors will now run at the speed given by your muscle contractions! Cool, right?
    ...
    //driveArdumoto(MOTOR_A, CCW, LeftMotorValue);
    //driveArdumoto(MOTOR_B, CCW, RightMotorValue);
 ...
 *  control. Students shouldn't mess with them.
 *  -------------------------------------------------------------
 */
